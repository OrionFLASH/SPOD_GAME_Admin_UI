...
| Колонка | Назначение |
|---------|------------|
| `REWARD_CODE` | Уникальный код награды; используется в связке `REWARD-LINK.REWARD_CODE`. |
...
| Колонка | Назначение |
|---------|------------|
| `CONTEST_CODE` | Уникальный код конкурса; ключ связи с `GROUP`, `INDICATOR`, `SCHEDULE`, `REWARD-LINK`, `REPORT`. |
...
| `BUSINESS_STATUS` | Бизнес-статус конкурса (например `АКТИВНЫЙ`/`АРХИВНЫЙ`). |
| `CONTEST_TYPE` | Тип конкурса; влияет на структуру `CONTEST_FEATURE`. |
| `CONTEST_DESCRIPTION` | Текстовое описание условий конкурса. |
| `CONTEST_FEATURE` | JSON с параметрами отображения, рассылок, фильтров и выдачи наград. |
| `SHOW_INDICATOR` | Подпись/единица отображения целевого показателя. |
| `PRODUCT_GROUP` | Бизнес-направление (продуктовая группа). |
...
| `TARGET_TYPE` | Вид цели расчёта (в выгрузках встречаются `ПРОМ`/`ТЕСТ`). |
...
| Колонка | Назначение |
|---------|------------|
| `CONTEST_CODE` | Код конкурса (внешний ключ на `CONTEST.CONTEST_CODE`). |
| `GROUP_CODE` | Код уровня группировки (`BANK`, `TB`, `GOSB`, `GROUPING`). |
| `GROUP_VALUE` | Значения уровня (в т.ч. `*` или массив кодов оргструктуры). |
| `GET_CALC_METHOD` | Метод определения победителей (`1` — процент, `2` — TOP-N, `3` — по критерию). |
| `GET_CALC_CRITERION` | Основной порог/параметр отбора победителей. |
| `ADD_CALC_CRITERION` | Дополнительный критерий отбора участников. |
| `ADD_CALC_CRITERION_2` | Второй дополнительный критерий отбора участников. |
| `BASE_CALC_CODE` | Базовый уровень для расчёта критериев (`BANK`/`TB`/`GOSB`/`GROUPING`). |
...
| `INDICATOR_FILTER` | JSON-фильтр операций/атрибутов, участвующих в расчёте. |
| `CONTESTANT_SELECTION` | Правило выбора участника (`0` — на дату операции, `1` — на дату окончания турнира). |
| `CALC_TYPE` | Тип расчёта (кодовое значение). |
...
| Колонка | Назначение |
|---------|------------|
| `CONTEST_CODE` | Код конкурса (ссылка на `CONTEST.CONTEST_CODE`). |
| `GROUP_CODE` | Код уровня конкурса (`BANK`/`TB`/`GOSB`/`GROUPING`). |
| `REWARD_CODE` | Код награды (ссылка на `REWARD.REWARD_CODE`) для указанной связки конкурса и уровня. |
...
| Колонка | Назначение |
|---------|------------|
| `TOURNAMENT_CODE` | Уникальный код турнира/периода в конкурсе. |
| `PERIOD_TYPE` | Тип периодичности турнира (месяц/квартал/произвольный и т.п.). |
| `START_DT` | Дата начала основного периода турнира. |
| `END_DT` | Дата окончания основного периода турнира. |
...
| `CONTEST_CODE` | Код родительского конкурса (ссылка на `CONTEST`). |
| `TARGET_TYPE` | JSON с параметрами цели турнира (в т.ч. `seasonCode`); см. разбор ниже. |
| `CALC_TYPE` | Тип расчёта турнира (промышленный/ручной, код). |
